...
def test_core_registers_builtin_providers():
    ...
    assert "storage" in app.registry.provider_names()
    assert "gateway" in app.registry.provider_names()
    ...
    assert "open-webui" in app.registry.provider_names()
    assert "backups" in app.registry.provider_names()
